...
1. 在 Editor 中打开任意一张横图的“照片设置”；竖图不会显示封面按钮。
2. 点击“设为首页封面”，再选择第 1–6 张中的一个位置。
3. 新图片会替换该位置原有图片；如果它已经在轮播中，则两个位置会直接交换。
4. 关闭照片窗口后点击 Editor 顶部的“保存”，设置会写入 `src/data/site.json` 的 `homeIntroImages`。
